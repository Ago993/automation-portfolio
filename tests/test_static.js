...
assert.ok(html.includes("03 / WORKFLOW"));
assert.ok(html.includes("SU MISURA"));
assert.ok(html.includes("€149")&&html.includes("€229")&&html.includes("€349"));
for(const demo of ["ImportFlow","ReportFlow","ListinoDiff","ReconcileFlow","QuoteFlow"]) {
  assert.ok(html.includes(demo),demo+" pricing example missing");
}
assert.ok(html.includes('class="custom-offer'));
assert.ok(html.includes("reportflow-clean.png"));
assert.ok(html.includes("listinodiff-clean.png"));
